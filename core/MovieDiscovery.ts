...
export function findMovieInfo(query: string): Promise<Movie> {
  return getImdbId(query).then(imdbId => getMovieInfoByImdbId(imdbId));
}

export function getImdbId(query: string): Promise<string> {
  return Http.get(`http://www.google.com/search?q=imdb+${cleanTitle(query) }`)
...
export function getMovieInfoByImdbId(imdbId: string): Promise<Movie> {
...
function prepare(movie: any): Movie {
  return fixArrays(toCamelCase(movie));
}

function fixArrays(movie: any): Movie {
...
function toCamelCase(obj: any): any {
  ...
  for (var key in obj) {
    if (obj.hasOwnProperty(key)) {
      result[firstToLower(key)] = obj[key];
    }
  }
  return result;
}
...
function cleanTitle(dirtyTitle: string): string {
  let dt = dirtyTitle.toLowerCase();
  return _replacables.reduce((prev, curr) => prev.split(curr).join(""), dt);
}
